...
🔍`INPUT[text(placeholder(输入搜索词),class(text-30)):searchTerm1]`  📁`INPUT[text(placeholder(输入搜索路径|默认Documents),class(text-40)):searchPath1]` 
```dataviewjs
// 获取当前页面的元数据
const currentPage = dv.current()
// 从 Meta Bind 输入获取搜索关键词、路径和精确搜索设置
const term = currentPage.searchTerm1 ?? ""
const folderpath = currentPage.searchPath1 ?? "Documents"
...
if (term) {
    ...
    // 检查文本是否匹配搜索词
    function isMatch(text, searchTerm1, precise) {
        // 检查是否搜索的是标题格式（以#开头）
        const headerSearch = searchTerm1.match(/^(#{1,6})\s*/)
        ...
        if (headerSearch) {
            ...
            const searchContent = searchTerm1.substring(searchLevel).trim()
            if (precise) {
        ...
        if (precise) {
            return text === searchTerm1
        } else {
            return text.includes(searchTerm1)
        }
    }
