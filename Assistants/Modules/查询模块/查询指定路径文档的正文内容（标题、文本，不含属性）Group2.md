...
🔍`INPUT[text(placeholder(输入搜索词),class(text-30)):searchTerm2]`  📁`INPUT[text(placeholder(输入搜索路径|默认Documents),class(text-40)):searchPath2]` 
```dataviewjs
// 获取当前页面的元数据
const currentPage = dv.current()
// 从 Meta Bind 输入获取搜索关键词、路径和精确搜索设置
const term = currentPage.searchTerm2 ?? ""
const folderpath = currentPage.searchPath2 ?? "Documents"
...
if (term) {
    ...
    // 检查文本是否匹配搜索词
    function isMatch(text, searchTerm2, precise) {
        // 检查是否搜索的是标题格式（以#开头）
        const headerSearch = searchTerm2.match(/^(#{1,6})\s*/)
        ...
        if (headerSearch) {
            ...
            const searchContent = searchTerm2.substring(searchLevel).trim()
            if (precise) {
        ...
        if (precise) {
            return text === searchTerm2
        } else {
            return text.includes(searchTerm2)
        }
    }
